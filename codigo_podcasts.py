# Convertendo: gramatike_app/templates/podcasts.html
...
async def _podcasts_page(self, db, current_user):
    ...
    .topics-bar a:hover {{ background:#9B5DE5; color:#fff; border-color:#9B5DE5; }}
        ...
        .podcast-title {{ font-size:.9rem; font-weight:800; letter-spacing:.45px; color:#9B5DE5; margin:0 0 .45rem; }}
    ...
    /* Evitar que o card se mova quando o menu está aberto e evitar que a mídia capture cliques acima do menu */
    ...
    @media (max-width: 980px){{ footer {{ display:none !important; }} }}
        ...
        #editPodcastForm label {{ font-size:.65rem; font-weight:800; letter-spacing:.6px; text-transform:uppercase; color:#9B5DE5; }}
        ...
        #editPodcastForm button#ep_cancel {{ background:#f1edff; color:#9B5DE5; }}
        #editPodcastForm button#ep_cancel:hover {{ background:#e4d7fb; }}
        #editPodcastForm button.nav-button {{ background:#9B5DE5; }}
    ...
    <header class="site-head">
        ...
        {{% if current_user.is_authenticated and (current_user.is_admin or current_user.is_superadmin) %}}
            <a href="{{{{ url_for('admin.dashboard') }}}}" style="position:absolute; top:14px; right:16px; background:#ffffff22; border:1px solid #ffffff42; color:#fff; padding:6px 10px; font-size:11px; font-weight:700; border-radius:8px; text-decoration:none; letter-spacing:.5px;">🛠️ Painel</a>
        {{% endif %}}
    ...
    <main>
        ...
        <div class="search-box">
            <form method="get" action="">
                <input type="text" name="q" value="{{{{ q }}}}" placeholder="Pesquisar título ou descrição..." />
    ...
    <!-- Lista -->
        <ul class="podcast-list">
            ...
            <li class="podcast-item" id="topico-{{{{ c.id }}}}">
                ...
                {{# Garante que variáveis não vazem entre itens #}}
                {{% set embed_src = None %}}
                {{% set sid = None %}}
                {{% set kind = None %}}
                {{% if current_user.is_authenticated and (current_user.is_admin or current_user.is_superadmin) %}}
                ...
                {{% if extra and (extra.spotify_embed or extra.spotify or extra.spotify_url or extra.spotify_link) %}}
                    ...
                    {{% else %}}
                        ...
                        {{% else %}}
                            <a href="{{{{ raw_sp }}}}" target="_blank" rel="noopener" style="font-size:.7rem;font-weight:700;display:inline-block;margin-top:.4rem;">Abrir no Spotify ▶</a>
                        {{% endif %}}
                    {{% endif %}}
                {{% elif c.url and ('spotify.com/' in (c.url|lower) or c.url.startswith('spotify:')) %}}
                    ...
                    {{% if embed_src %}}
                        <iframe title="Spotify" style="border-radius:12px; margin-top:.5rem;" src="{{{{ embed_src }}}}" width="100%" height="352" frameborder="0" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" allowfullscreen loading="lazy" referrerpolicy="strict-origin-when-cross-origin"></iframe>
                    {{% else %}}
                        <a href="{{{{ spot }}}}" target="_blank" rel="noopener" style="font-size:.7rem;font-weight:700;display:inline-block;margin-top:.4rem;">Abrir no Spotify ▶</a>
                    {{% endif %}}
                {{# Campo extra com áudio (audio_url, audio, audio_mp3) #}}
                ...
                {{# URL crua contendo extensão de áudio em qualquer posição (mesmo com query) #}}
                ...
                {{% else %}}
                    <span style="font-size:.65rem; color:#777;">Sem mídia.</span>
                {{% endif %}}
                                                {{% if c.resumo %}}<p>{{{{ c.resumo }}}}</p>{{% endif %}}
                                                {{% if c.extra and (c.extra | fromjson).author %}}
                                                    <p class="meta-author">Autore: {{{{ (c.extra | fromjson).author }}}}</p>
                                                {{% endif %}}
            </li>
            {{% else %}}
        ...
        <dialog id="editPodcastDialog">
            <form id="editPodcastForm" method="post">
                ...
                <label style="display:block; font-size:.8rem; margin:.4rem 0 .2rem;">Título</label>
                <input name="titulo" id="ep_titulo" required />
                <label style="display:block; font-size:.8rem; margin:.6rem 0 .2rem;">Autore</label>
                ...
                <label style="display:block; font-size:.8rem; margin:.6rem 0 .2rem;">Tópico</label>
        ...
        <div style="margin-top:2.2rem; display:flex; justify-content:center; gap:.65rem; flex-wrap:wrap;">
            {{% if page and last_page %}}
                ...
                {{% if page > 1 %}}
                    <a href="{{{{ url_for('main.podcasts') }}}}{{{{ base_q }}}}{{{{ '&' if base_q else '?' }}}}page=1" style="text-decoration:none; font-size:.65rem; font-weight:800; letter-spacing:.5px; padding:.55rem .9rem .5rem; background:#f1edff; border:1px solid #e2d6f9; color:#9B5DE5; border-radius:18px;">« Primeiro</a>
                    <a href="{{{{ url_for('main.podcasts') }}}}{{{{ base_q }}}}{{{{ '&' if base_q else '?' }}}}page={{{{ page-1 }}}}" style="text-decoration:none; font-size:.65rem; font-weight:800; letter-spacing:.5px; padding:.55rem .9rem .5rem; background:#f1edff; border:1px solid #e2d6f9; color:#9B5DE5; border-radius:18px;">‹ Anterior</a>
                {{% endif %}}
                <span style="font-size:.6rem; font-weight:800; letter-spacing:.55px; color:#7d3dc9; padding:.55rem .8rem .5rem; background:#fff; border:1px solid #e2d6f9; border-radius:18px;">Página {{{{ page }}}} / {{{{ last_page }}}}</span>
                {{% if page < last_page %}}
                    <a href="{{{{ url_for('main.podcasts') }}}}{{{{ base_q }}}}{{{{ '&' if base_q else '?' }}}}page={{{{ page+1 }}}}" style="text-decoration:none; font-size:.65rem; font-weight:800; letter-spacing:.5px; padding:.55rem .9rem .5rem; background:#f1edff; border:1px solid #e2d6f9; color:#9B5DE5; border-radius:18px;">Próxima ›</a>
                    <a href="{{{{ url_for('main.podcasts') }}}}{{{{ base_q }}}}{{{{ '&' if base_q else '?' }}}}page={{{{ last_page }}}}" style="text-decoration:none; font-size:.65rem; font-weight:800; letter-spacing:.5px; padding:.55rem .9rem .5rem; background:#f1edff; border:1px solid #e2d6f9; color:#9B5DE5; border-radius:18px;">Última »</a>
                {{% endif %}}
            {{% endif %}}
        </div>
    </main>
    <footer>
        © 2025 Gramátike • Linguagem Viva e de Todes
    </footer>
    <script>
    (function(){{
        const list = document.querySelector('.podcast-list');
