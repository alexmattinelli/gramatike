# Convertendo: gramatike_app/templates/dinamicas.html
...
async def _dinamicas_page(self, db, current_user):
    ...
    main {{ width:100%; max-width:980px; margin:2rem auto 4rem; padding:0 clamp(16px,4vw,40px); overflow-x:hidden; }}
    ...
    label {{ display:block; margin-bottom:.3rem; font-weight:800; color:#9B5DE5; font-size:.85rem; }}
    ...
    @media (max-width: 768px) {{
      ...
      .builder-card {{ padding:.6rem; max-width:100%; box-sizing:border-box; overflow-wrap:break-word; word-break:break-word; }}
      .builder-actions {{ flex-direction:column; gap:.5rem; }}
      .builder-actions .btn {{ width:100%; box-sizing:border-box; }}
      
      /* Fix form layout on mobile */
      #dynCreateForm {{ gap:.8rem; }}
      #dynCreateForm > div {{ max-width:100%; overflow-x:hidden; box-sizing:border-box; }}
      #dynCreateForm input, #dynCreateForm select, #dynCreateForm textarea {{ 
        max-width:100%; 
        box-sizing:border-box;
        font-size:16px; /* Prevent zoom on iOS */
      }}
      .opt-row {{ flex-wrap:wrap; gap:.3rem; }}
      .chip {{ font-size:.7rem; padding:.3rem .5rem; word-break:break-word; }}
    }}
  ...
  <main>
    ...
    {{% else %}}
      <div class="card" style="margin-bottom:1rem;">
        <h2 style="margin:0 0 1rem; font-size:1.3rem; color:#9B5DE5; font-weight:700;">Criar dinâmica</h2>
        <form id="dynCreateForm" method="POST" action="{{{{ url_for('main.dinamicas_create') }}}}" enctype="multipart/form-data" style="display:grid; gap:.9rem; grid-template-columns:1fr; font-family:'Nunito', system-ui, -apple-system, 'Segoe UI', Roboto, Arial, sans-serif;">
          ...
          <div id="poll_opts">
            <label>Opções (1 por linha)</label>
            <textarea name="opcoes" rows="4" placeholder="Opção A&#10;Opção B"></textarea>
          </div>
          <div id="form_builder" style="display:none;">
            <div class="muted-sm" style="margin-bottom:.6rem;">Adicione perguntas como no Google Forms. Tipos: resposta curta, parágrafo, múltipla escolha.</div>
            <div id="q_list"></div>
          ...
          <div id="quemsouleu_builder" style="display:none;">
            <div class="muted-sm" style="margin-bottom:.6rem;">Adicione frases ou fotos que os participantes terão que responder. Escolha o que eles devem descobrir (gênero, orientação, pronome, etc.).</div>
            <div>
              <label style="margin-top:.8rem;">O que a pessoa deve descobrir?</label>
              <input type="text" name="questao_tipo" id="questao_tipo" placeholder="Ex: gênero, orientação sexual, pronome..." />
            </div>
            <div>
              <label style="margin-top:.8rem;">Moral/Mensagem Final</label>
          ...
          <button type="submit" class="btn primary" style="margin-top:.5rem;">Criar Dinâmica</button>
        </form>
      </div>
      <div class="card">
        <h3 style="margin:.2rem 0 .8rem; color:#9B5DE5; font-weight:700;">Minhas dinâmicas</h3>
        {{% if (my_dynamics or [])|length == 0 %}}
          <p class="muted">Nenhuma dinâmica criada ainda.</p>
        {{% else %}}
          <div class="grid">
            {{% for d in my_dynamics %}}
              <div class="builder-card" style="display:flex; flex-direction:column; gap:.5rem;">
                <div style="display:flex; align-items:flex-start; justify-content:space-between; gap:.5rem; flex-wrap:wrap;">
                  <div style="font-weight:800; color:#222; flex:1; min-width:0;">{{{{ d.titulo }}}}</div>
                ...
                {{% if d.descricao %}}<div class="muted-sm" style="word-break:break-word;">{{{{ d.descricao }}}}</div>{{% endif %}}
                <div style="display:flex; gap:.4rem; flex-wrap:wrap; margin-top:.35rem;">
                  <a class="btn" href="{{{{ url_for('main.dinamica_admin', dyn_id=d.id) }}}}" style="font-size:.75rem; padding:.45rem .75rem;">Ver respostas</a>
                  <a class="btn" href="{{{{ url_for('main.dinamica_view', dyn_id=d.id) }}}}" style="font-size:.75rem; padding:.45rem .75rem;">Abrir</a>
                  <a class="btn" href="{{{{ url_for('main.dinamica_export_csv', dyn_id=d.id) }}}}" style="font-size:.75rem; padding:.45rem .75rem;">Baixar CSV</a>
                  <a class="btn" href="{{{{ url_for('main.dinamica_edit', dyn_id=d.id) }}}}" style="font-size:.75rem; padding:.45rem .75rem;">Editar</a>
                  <form method="POST" action="{{{{ url_for('main.dinamica_toggle_active', dyn_id=d.id) }}}}" style="display:inline; margin:0;">
                    
                    <button type="submit" class="btn" style="font-size:.75rem; padding:.45rem .75rem; {{% if d.active %}}background:#28a745;color:#fff;{{% else %}}background:#dc3545;color:#fff;{{% endif %}}">
                      {{% if d.active %}}Finalizar{{% else %}}Ativar{{% endif %}}
                    </button>
                  </form>
                  <form method="POST" action="{{{{ url_for('main.dinamica_delete', dyn_id=d.id) }}}}" style="display:inline; margin:0;" onsubmit="return confirm('Tem certeza que deseja excluir esta dinâmica? Esta ação não pode ser desfeita.');">
                    
                    <button type="submit" class="btn" style="font-size:.75rem; padding:.45rem .75rem; background:#dc3545;color:#fff;">
                      Excluir
                    </button>
                  </form>
                </div>
        ...
        card.innerHTML = `
          <div class="row">
            ...
            <div style="margin-top:.5rem;">
              <label style="font-weight:600; display:block;">Resposta Correta Principal</label>
            ...
            <div style="margin-top:.5rem;">
              <label style="font-weight:600; display:block;">Respostas Alternativas (opcional)</label>
              <input type="text" data-k="alternativas" value="${{(item.alternativas||[]).join(', ')}}" style="width:100%;" placeholder="Ex: nb, nao binario, enby" />
