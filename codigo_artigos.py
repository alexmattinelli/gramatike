# Convertendo: gramatike_app/templates/artigos.html
...
async def _artigos_page(self, db, current_user):
    ...
    .edu-nav a {{ text-decoration:none; font-weight:700; font-size:.7rem; letter-spacing:.55px; padding:.65rem 1.05rem .62rem; background:#ffffff18; color:#fff; border:1px solid #ffffff30; backdrop-filter:blur(4px); -webkit-backdrop-filter:blur(4px); border-radius:22px; display:inline-flex; align-items:center; gap:.35rem; box-shadow:0 4px 12px rgba(0,0,0,.18); transition:.25s; font-family:'Nunito',system-ui,-apple-system,'Segoe UI',Roboto,Arial,sans-serif; }}
        ...
        .page-title {{ text-align:center; font-size:2.1rem; color:#9B5DE5; margin:0 0 1.6rem; }}
    ...
    .search-box button:hover {{ filter:brightness(1.07); }}
        .artigos-ul {{ list-style:none; margin:0; padding:0; display:grid; gap:1.35rem; grid-template-columns:repeat(auto-fit,minmax(340px,1fr)); }}
    .artigo {{ background:var(--card); border:1px solid var(--border); border-radius:26px; padding:1.1rem 1.2rem 1rem; position:relative; box-shadow:0 10px 24px -6px rgba(0,0,0,.10); transition:.3s; z-index:0; }}
    /* Quando menu aberto eleva o card acima dos vizinhos evitando que o dropdown fique coberto */
    .artigo.menu-open {{ z-index:999; }}
    /* Gradiente removido a pedido (antes existia pseudo-elemento ::before) */
    /* .artigo::before {{ content:""; position:absolute; inset:0; pointer-events:none; background:none; }} */
        .artigo:hover {{ box-shadow:0 18px 46px -10px rgba(0,0,0,.28); transform:translateY(-3px); }}
        .artigo a {{ text-decoration:none; font-weight:800; letter-spacing:.4px; color:#9B5DE5; font-size:.9rem; }}
        .artigo a:hover {{ text-decoration:underline; }}
        .artigo .meta {{ margin:.45rem 0 0; font-size:.7rem; line-height:1.3; color:var(--text-dim); font-weight:600; }}
    ...
    .pag-btn.disabled {{ pointer-events:none; opacity:.45; }}
        @media (max-width:720px){{ header.site-head {{ padding:12px 18px 18px; }} .logo {{ font-size:1.5rem; }} .artigos-ul {{ grid-template-columns:repeat(auto-fit,minmax(240px,1fr)); }} }}
    ...
    <header class="site-head">
        ...
        <nav class="edu-nav" aria-label="Seções educacionais">
            ...
            #}}
            <a href="{{{{ url_for('main.artigos') }}}}" class="active">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
                    <polyline points="14 2 14 8 20 8"></polyline>
                    <line x1="16" y1="13" x2="8" y2="13"></line>
    ...
    <!-- Título removido a pedido -->
                <div class="search-box">
                    <form method="get" action="">
                        <input type="text" name="q" value="{{{{ q }}}}" placeholder="Pesquisar título ou resumo..." />
                ...
                {{% endif %}}
        <!-- Formulário para admin adicionar novos artigos -->
        <!-- Lista de artigos disponíveis -->
    <div class="artigos-lista">
                                    {{% if conteudos %}}
                        <ul class="artigos-ul" style="list-style:none; padding-left:0;">
                                    {{% for c in conteudos %}}
                                            {{% set extra = (c.extra | fromjson) if c.extra else None %}}
                                            <li class="artigo" id="topico-{{{{ c.id }}}}">
                                                    {{% if current_user.is_authenticated and (current_user.is_admin or current_user.is_superadmin) %}}
                                                    ...
                                                    <div class="item-menu" role="menu">
                                                        ...
                                                        <button type="button" data-share="{{{{ c.id }}}}">Compartilhar</button>
                                                        <form method="POST" action="/admin/edu/content/{{{{ c.id }}}}/delete" onsubmit="return confirm('Excluir este artigo?');">
                ...
                {{% else %}}
                        <p style="text-align:center; color:#888;">Nenhum artigo disponível no momento.</p>
                {{% endif %}}
        </div>
        {{% if last_page and last_page > 1 %}}
        <div class="pagination" style="margin:2.2rem 0 0; display:flex; gap:.6rem; justify-content:center; align-items:center; flex-wrap:wrap; font-size:.7rem; font-weight:700; letter-spacing:.4px;">
            {{% set base_params = '' %}}
    ...
    {{% if current_user.is_authenticated and (current_user.is_admin or current_user.is_superadmin) %}}
    <dialog id="editArtigoDialog" style="border:none; border-radius:20px; padding:0; max-width:600px; width:90%;">
        <form id="editArtigoForm" method="post" style="display:grid; gap:.9rem; padding:1.5rem;">
            <h3 style="margin:0; font-size:1.3rem; color:#9B5DE5;">Editar Artigo</h3>
            
            <input type="hidden" name="content_id" id="ea_id" />
            
            <label style="display:grid; gap:.3rem;">
                <span style="font-size:.75rem; font-weight:700; color:#666;">Título</span>
                <input name="titulo" id="ea_titulo" required style="border:1px solid #cfd7e2; border-radius:10px; padding:.65rem .75rem; font-size:.85rem;" />
            </label>
            
            <label style="display:grid; gap:.3rem;">
                <span style="font-size:.75rem; font-weight:700; color:#666;">Autore</span>
                <input name="autor" id="ea_autor" style="border:1px solid #cfd7e2; border-radius:10px; padding:.65rem .75rem; font-size:.85rem;" />
            </label>
            
            <label style="display:grid; gap:.3rem;">
                <span style="font-size:.75rem; font-weight:700; color:#666;">Resumo</span>
                <textarea name="resumo" id="ea_resumo" rows="8" style="min-height:200px; resize:vertical; border:1px solid #cfd7e2; border-radius:10px; padding:.65rem .75rem; font-size:.85rem;"></textarea>
            </label>
            
            <label style="display:grid; gap:.3rem;">
                <span style="font-size:.75rem; font-weight:700; color:#666;">URL</span>
                <input name="url" id="ea_url" style="border:1px solid #cfd7e2; border-radius:10px; padding:.65rem .75rem; font-size:.85rem;" />
            </label>
            
            <label style="display:grid; gap:.3rem;">
                <span style="font-size:.75rem; font-weight:700; color:#666;">Tópico</span>
                <select name="topic_id" id="ea_topic" style="border:1px solid #cfd7e2; border-radius:10px; padding:.65rem .75rem; font-size:.85rem;">
            ...
            <menu style="display:flex; gap:.6rem; justify-content:flex-end; margin:0; padding-top:.6rem;">
                <button type="button" id="ea_cancel" style="padding:.65rem 1.2rem; border:1px solid #cfd7e2; background:#f9f9f9; border-radius:12px; font-weight:700; cursor:pointer;">Cancelar</button>
    ...
    <script>
    (function(){{
        const list = document.querySelector('.artigos-ul');
        if(!list) return;
        
        // Editar via modal
        const dlg = document.getElementById('editArtigoDialog');
        const form = document.getElementById('editArtigoForm');
        const ea_id = document.getElementById('ea_id');
        const ea_titulo = document.getElementById('ea_titulo');
        const ea_resumo = document.getElementById('ea_resumo');
        const ea_url = document.getElementById('ea_url');
        const ea_topic = document.getElementById('ea_topic');
        const ea_cancel = document.getElementById('ea_cancel');
        const ea_autor = document.getElementById('ea_autor');
        ...
        list.addEventListener('click', async (e)=>{{
            ...
            if(editBtn){{
                ...
                try{{
                    ...
                    ea_id.value = data.id;
                    ea_titulo.value = data.titulo||'';
                    ea_resumo.value = data.resumo||'';
                    ea_autor.value = (data.extra && data.extra.author) ? data.extra.author : '';
                    ea_url.value = data.url||'';
                    [...ea_topic.options].forEach(o=>{{ o.selected = String(o.value)===String(data.topic_id||''); }});
                    dlg.showModal();
            ...
            if(shareBtn){{
                ...
                const url = `${{window.location.origin}}${{window.location.pathname}}#topico-${id}`;
        ...
        ea_cancel.addEventListener('click', ()=> dlg.close());
        form.addEventListener('submit', async (e)=>{{
            e.preventDefault();
            const id = ea_id.value;
        ...
        document.addEventListener('click', (e)=>{{
            if(!e.target.closest('.artigo')){{
                document.querySelectorAll('.item-menu.show').forEach(m=>m.classList.remove('show'));
            }}
        }});
    }})();
    
    // Ver mais / Ver menos resumo
